...
#    Updated: 2021/05/25 20:21:54 by lwourms          ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
RUN openssl req -x509 -nodes -days 30 -subj "/C=FR/ST=France/L=Lyon/O=42 Network/OU=Winkel/CN=Ludwig Wourms" -newkey rsa:2048 -keyout /etc/ssl/nginx-selfsigned.key -out /etc/ssl/nginx-selfsigned.crt;
#----------------------------------- 5. PHP MY ADMIN ---------------------------------------------
# Move start.sh, auto_index.sh, default and defaultoff from our computer inside the container
COPY ./srcs/start.sh ./
